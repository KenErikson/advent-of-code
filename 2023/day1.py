...
numbersStr = {"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"}
numbersStrMap = {"one":1, "two":2, "three":3, "four":4, "five":5, "six":6, "seven":7, "eight":8, "nine":9}
for row in input:
    firstnum = -1
    lastnum = -1
    for i in range(len(row)):
        if row[i] in numbers:
            ...
            for possibleNum in numbersStr:
                if row[i:].startswith(possibleNum):
                    if firstnum == -1:
                        firstnum = numbersStrMap[possibleNum]
                    lastnum = numbersStrMap[possibleNum]
    if lastnum == -1 or firstnum == -1:
        print("ERROR")
        print(row)
    else:
